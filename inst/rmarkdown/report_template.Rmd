...
if (ncnum > 0) {
  ...
  scan_numerics(data) %>%
  kable("html", escape = F, align = c("l", rep("r", 9))) %>% 
  kable_styling(full_width = F, bootstrap_options = "condensed")
}
```

```{r ifnumplot, fig.width = 10, out.width = '100%',fig.asp = 0.8*ceiling(ncnum/4)}
if (ncnum > 0) {
...
```{r ifcatplot, fig.width = 10, out.width = '100%', fig.asp = 0.8*ceiling(nccat/4)}
if (nccat > 0) {
...
```{r num-num, fig.width = 10, out.width = '100%', fig.asp = 0.8*ceiling(ncnum*(ncnum - 1)/8)}
if (ncnum >= 2) {
...
```{r num-cat, fig.width = 10, out.width = '100%', fig.asp = 0.8*ceiling(ncnum*nccat/8)}
if (nccat > 0 & ncnum > 0) {
  plotnumcat <- vis_ngcovar(data)
...
```{r cat-cat, fig.width = 10, out.width = '100%', fig.asp = 0.8*ceiling(nccat*(nccat - 1)/8)}
if (nccat >= 2) {
  plotcatcat <- vis_ggcovar(data)
